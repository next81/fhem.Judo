...
my $controls_file = 'controls_Judo.txt';
my @expected = ('FHEM/50_Judo.pm');

# Ermittelt dieselbe plattformunabhaengige LF-Bytelaenge wie der Generator.
sub delivery_size($) {
	my ($file) = @_;
...
# Alle Produktionsbibliotheken muessen im Controlfile enthalten sein.
find(
	{
		no_chdir => 1,
		wanted => sub {
...
# Jede Zeile wird formal geprueft und genau einem Pfad zugeordnet.
for my $line (@lines) {
	chomp $line;
...
# Existenz und Bytelaenge schuetzen vor unvollstaendigen Releases.
for my $path (sort keys %entries) {
	ok(-f $path, "$path existiert");
	is($entries{$path}{size}, delivery_size($path), "$path hat die angegebene Auslieferungs-Bytelaenge");
}
